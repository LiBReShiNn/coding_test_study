...
class Kakao2022_le1_2 {

    private boolean validate(String[] id_list, String[] report, int k) {
    ...
    private List<String> makeReporterCollection(String[] report) {
    ...
    public int[] solution(String[] id_list, String[] report, int k) {
        ...
        List<String> totalBads = makeReporterCollection(report);

        caluateBads(totalBads);
        // 아이디, 신고 횟수
        Map<String, Integer> badCnt = new HashMap<>(); // "b" : 1, "c" : 2 ...
        // k번 이상 신고 당한 사람 목록
...
//        }
        ...
        for (int i = 0; i < id_list.length; i++) {
            if (badIds.contains(id_list[i])) {
                rt[i] = badCnt.get(id_list[i]);
            } else {
                rt[i] = 0;
            }
        }
    ...
    private void caluateBads(List<String> totalBads) {
    }
